...
//	@Test
    ...
    void testWithIdentifiers() {
        ...
        if(!new File('src/test/data/samples-with-identifiers.txt').exists())
            return
        
        def headers = [
            'Batch','Sample_ID','DNA_Tube_ID','Sex','DNA_Concentration','DNA_Volume','DNA_Quantity','DNA_Quality','DNA_Date','Cohort','Sample_Type','Fastq_Files','Prioritised_Genes','Consanguinity','Variants_File','Pedigree_File', 'Identifiers', 'Test_ID'
        ]
        
        Map info = SampleInfo.parse_sample_info('src/test/data/samples-with-identifiers.txt', headers)
    }
}
